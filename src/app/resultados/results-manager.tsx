...
import { Award, Check, Plus, Send, X } from "lucide-react";
import { createClient } from "@/lib/supabase/client";

type Category = { id: string; name: string };
type EventItem = { id: string; name: string; competitions: Array<{ id: string; name: string; categories: Category[] }> };
type Registration = { id: string; event_id: string; category_id: string; participants: { full_name: string }[] | null };
type Result = { id: string; category_id: string; registration_id: string; position: number | null; total: number | null; published_at: string | null };
...
export default function ResultsManager({ initialEvents, initialRegistrations, initialResults }: { initialEvents: EventItem[]; initialRegistrations: Registration[]; initialResults: Result[] }) {
  ...
  const categories = event?.competitions.flatMap((competition) => competition.categories.map((category) => ({ ...category, competition: competition.name }))) || [];
  ...
  async function saveResult(event: FormEvent<HTMLFormElement>) {
    ...
    if (!currentCategoryId || !registrationId || !Number.isInteger(position) || position < 1) { setError("Informe participante e posição válida."); setLoading(false); return; }
    ...
    const payload = { position, total };
    const query = existing ? supabase.from("results").update(payload).eq("id", existing.id).select("id,category_id,registration_id,position,total,published_at").single() : supabase.from("results").insert({ ...payload, category_id: currentCategoryId, registration_id: registrationId }).select("id,category_id,registration_id,position,total,published_at").single();
    const { data, error: saveError } = await query;
    if (saveError || !data) { setError(saveError?.message || "Não foi possível salvar o resultado."); setLoading(false); return; }
  ...
  async function togglePublication() {
    ...
    if (updateError) setError(updateError.message); else setResults((items) => items.map((item) => item.category_id === currentCategoryId ? { ...item, published_at: publishedAt } : item));
    setLoading(false);
  }

  return <section className="results-workspace"><div className="result-controls"><label>Evento<select value={eventId} onChange={(event) => { setEventId(event.target.value); setCategoryId(""); }}>{initialEvents.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Categoria<select value={currentCategoryId} onChange={(event) => setCategoryId(event.target.value)}>{categories.map((category) => <option value={category.id} key={category.id}>{category.competition} · {category.name}</option>)}</select></label><button className="primary" onClick={() => setOpen(true)} disabled={!registrations.length}><Plus />Registrar resultado</button></div>{error && <div className="form-error">{error}</div>}{!initialEvents.length ? <div className="empty-state compact"><Award /><h2>Nenhum evento disponível</h2><p>Crie um evento, categorias e inscrições para registrar os resultados.</p></div> : !categories.length ? <div className="empty-state compact"><Award /><h2>Nenhuma categoria cadastrada</h2><p>Configure as categorias do evento antes de montar a classificação.</p></div> : <><div className="results-head"><div><span className="eyebrow">CLASSIFICAÇÃO ATUAL</span><h2>{categories.find((item) => item.id === currentCategoryId)?.name}</h2></div>{categoryResults.length > 0 && <button className={published ? "secondary published" : "primary"} onClick={togglePublication} disabled={loading}>{published ? <><Check />Publicado</> : <><Send />Publicar resultado</>}</button>}</div>{categoryResults.length ? <div className="results-list">{categoryResults.map((result) => <article key={result.id}><strong>{result.position || "—"}º</strong><div><b>{participantName(result.registration_id)}</b><span>{result.total === null ? "Sem pontuação registrada" : `${result.total.toFixed(2)} pontos`}</span></div><small>{result.published_at ? "Publicado" : "Rascunho"}</small></article>)}</div> : <div className="empty-state compact"><Award /><h2>Classificação ainda vazia</h2><p>{registrations.length ? "Registre as posições finais dos participantes desta categoria." : "Ainda não há participantes inscritos nesta categoria."}</p></div>}</>}{open && <div className="modal-wrap"><button className="backdrop" onClick={() => setOpen(false)} aria-label="Fechar" /><form className="modal category-modal" onSubmit={saveResult}><button type="button" className="modal-x" onClick={() => setOpen(false)}><X /></button><span className="eyebrow">{event?.name}</span><h2>Registrar resultado</h2><div className="form-grid"><label className="wide">Participante<select name="registration_id" required><option value="">Selecione...</option>{registrations.map((registration) => <option key={registration.id} value={registration.id}>{participantName(registration.id)}</option>)}</select></label><label>Posição<input name="position" type="number" min="1" required /></label><label>Pontuação total<input name="total" type="number" min="0" step="0.01" placeholder="Opcional" /></label></div>{error && <div className="form-error">{error}</div>}<div className="modal-actions"><button type="button" className="secondary" onClick={() => setOpen(false)}>Cancelar</button><button className="primary" disabled={loading}>{loading ? "Salvando..." : "Salvar resultado"}</button></div></form></div>}</section>;
}
